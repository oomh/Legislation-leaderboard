"""
Assembly Bills Table Builder

Builds and processes legislative tables from MinerU-extracted assembly bill tracker JSON.
"""
...
def build_assembly_bills(
    json_file_path: str = "data/mineru_output_bill_tracker_assembly/content_list_v2.json",
) -> Dict[str, Any]:
    """
    Build assembly bill tables from MinerU-extracted JSON.

    This function:
    1. Extracts table objects from the assembly bill tracker JSON file
    2. Converts HTML content to DataFrames
    3. Concatenates all DataFrames into a single combined DataFrame

    Args:
        json_file_path (str): Path to the JSON file containing extracted assembly bills.
    ...
    """
    try:
        logger.info(f"Building assembly bills tables from {json_file_path}")

        # Step 1: Extract tables from JSON
        tables = extract_tables_from_json(json_file_path)

        if not tables:
            warning_msg = "No tables found in assembly bills JSON"
