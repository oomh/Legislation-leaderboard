"""Senate Bills Sponsor Splitter

Normalises senate bill tracker rows so that each row has exactly one sponsor.

Bills are first partitioned into three non-overlapping subsets:
    1. Office-sponsored bills  — sponsor contains a role keyword (leader, chairperson, majority, minority)
...
def extract_office_sponsored_bills(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: Senate bills DataFrame with a 'sponsor' column.
...
def extract_multi_sponsored_bills(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: Senate bills DataFrame with office-sponsored rows removed.
...
def partition_senate_bills(
    ...
    """Split senate bills into three non-overlapping subsets.
    ...
    Args:
        df: Transformed senate bills DataFrame with a 'sponsor' column.
...
_CHAIRPERSON_PATTERN = re.compile(r"^\s*(?:the\s+)?chairperson", re.IGNORECASE)
...
def split_office_sponsors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Rows whose sponsor starts with 'Chairperson' or 'The Chairperson' are kept
    as-is because their committee names often contain ' and ' (e.g. 'Chairperson,
    Standing Committee On Agriculture, Livestock And Fisheries'), which would
    produce false splits.
...
def rebuild_senate_bills(
    ...
    log.info(f"Rebuilt senate bills: {len(result)} rows total")
    return result
