...
def extract_bill_trackers_and_committee(
    ...
    """
    log.info("Starting MinerU extraction workflow for bill trackers and committee leadership")

    results = {}

    # Extract Senate bill tracker
    log.info(f"Extracting Senate bill tracker from {bill_tracker_senate_url}")
    senate_result = mineru_workflow(bill_tracker_senate_url, api_key, bill_tracker_senate_dir)
    ...
    if senate_result:
        log.info(f"Senate bill tracker extraction complete: {len(senate_result['file_list'])} files")
    else:
    ...
    assembly_result = mineru_workflow(bill_tracker_assembly_url, api_key, bill_tracker_assembly_dir)
    ...
    if assembly_result:
        log.info(f"Assembly bill tracker extraction complete: {len(assembly_result['file_list'])} files")
    else:
    ...
    committee_result = mineru_workflow(committee_leadership_url, api_key, committee_leadership_dir)
    ...
    if committee_result:
        log.info(f"Committee leadership extraction complete: {len(committee_result['file_list'])} files")
    else:
